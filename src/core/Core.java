...
public class Core {
	...
	public static void main(String[] args) {
		...
		List<Class<MessageComposer>> pluginClasses = new ArrayList<>();

		for ( File indexFile : getIndexFiles() ) {
			pluginClasses.addAll(getPlugins(indexFile));
		}
		
		for ( Class<MessageComposer> pluginClass : pluginClasses ) {
			try {
				String message = pluginClass.newInstance().composeMessage();
	...
	public static List<Class<MessageComposer>> getPlugins(File indexFile) {
		List<Class<MessageComposer>> plugins = new ArrayList<>();
		...
		try {
			...
			while ((className = br.readLine()) != null) {
				plugins.add((Class<MessageComposer>) Class.forName(className));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
